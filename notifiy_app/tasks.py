...
@shared_task(bind=True)
def delete_expired_notifications(self,data):
    expiration_time = timezone.now() - timezone.timedelta(minutes=1)
    
    # Deleting notifications older than the expiration time
    expired = BroadcastNotification.objects.filter(broadcast_on__lte=expiration_time)
    print('################################################################')
    print(expired.values())
    expired.delete()
    
    return 'Deleted old notifications'
